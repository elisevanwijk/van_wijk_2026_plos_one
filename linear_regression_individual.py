...
class LinearRegressionIndividal:
    def __init__(self):
    ...
    def _run_regression(self, data, dependent_variables, independent_variables):
        ...
        for dependent_variable in dependent_variables:
            ...
            for independent_variable in independent_variables:
                x = add_constant(data[independent_variable])
                model = sm.OLS(
                    data[dependent_variable],
                    x,
                ).fit()
                results[independent_variable] = model
            resultss[dependent_variable] = results
    ...
    def report_classify_per_cohort_rm(self):
        for sheet_name, modelss in self.classify_per_cohort_rm().items():
            print(sheet_name)
            for _, models in modelss.items():
    ...
    def report_classify_per_cohort_rm_dm(self):
        for sheet_name, modelss in self.classify_per_cohort_rm_dm().items():
            print(sheet_name)
            for _, models in modelss.items():
                for _, model in models.items():
                    print(model.summary())
    ...
    def report_classify_rm_total(self):
        for _, models in self.classify_rm_total().items():
            for _, model in models.items():
                print(model.summary())
    ...
    def report_classify_rm_dm_total(self):
        for _, models in self.classify_rm_dm_total().items():
            for _, model in models.items():
                print(model.summary())
    ...
    def generate_rm_results(self):
        ...
        self.generate_results(model_runs, "rm")

    def generate_rm_dm_results(self):
        model_runs = self.classify_per_cohort_rm_dm()
        model_runs["RM DM Total"] = self.classify_rm_dm_total()
        self.generate_results(model_runs, "rm_dm")
    ...
    def generate_results(self, model_runs, label):
        ...
        dfs = {"Model results": model_results}

        write_excel(
            dfs, results_folder / f"linear_regression_individual_{label}.xlsx", False
        )
        for log_name, result in logs.items():
            pth = results_folder / f"{label}_ols_individual_logs" / f"{log_name}.txt"
            pth.parent.mkdir(exist_ok=True, parents=True)
            pth.write_text(result)

    def generate_log(self, model_runs):
        logs = {}
        for sheet, modelss in model_runs.items():
            results = []
            for dep, models in modelss.items():
    ...
    def get_model_result(self, models, label):
    ...
    def get_coef(self, model, label):
        ...
        conf = pd.concat([model.pvalues, model.conf_int()], axis=1)
        conf.columns = ["p-values", "5%", "95%"]
        conf["coef"] = params
        conf["se"] = model.bse
